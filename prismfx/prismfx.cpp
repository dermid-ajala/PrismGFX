...
// end of Hareware communications code
...
void PrismFX::bufferChar(uint8_t raw) {
	if(raw < fnt->first || raw > fnt->last)	return;	// no character defined
	uint8_t c = raw - fnt->first;					// set for GFX font bitmap range				// character code out of range?
